...
p <- ggplot(data, aes(Time,Discharge_Capacity))
...
lookback <- c(seq(150, records_per_day * 1, 60), records_per_day * 4, records_per_day * 6)
...
windows <- forecastML::create_windows(data_train,
                                      ...
                                      # window_start = as.POSIXct("2015-11-13 09:13:44", "%Y-%m-%d %H:%M:%S", tz = "UTC"),
                                      # window_stop = as.POSIXct("2015-11-13 13:21:02", "%Y-%m-%d %H:%M:%S",tz = "UTC")
...
plot(data_combined_r, data_actual = test, actual_indices = test$Time,panel.grid.major = element_blank(),
  ...
  geom_line(data = data_combined_r,
            aes(x = forecast_period, y = Discharge_Capacity_pred), color = "#276DC2") +
...
p <- ggplot(data, aes(Time, Discharge_Capacity))
p <-p+ xlab("Date") + ylab("Discharge_Capacity") + labs(color = NULL) +
  ggtitle("3-Day-Ahead Forecasted PER MINUTE Discharge_Capacity")
p+theme(
  # Hide panel borders and remove grid lines
  panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(),text = element_text(size =15))

ggsave(file = "3-Day-Ahead Forecasted PER MINUTE Discharge_Capacity.png", width = 9, height = 6)
